...
import WarningCircleIcon from "../../../shared/icons/warning-circle";
import EllipsisVerticalIcon from "../../../shared/icons/ellipsis-vertical-icon";
import ArrowUpIcon from "../../../shared/icons/arrow-up-icon";
import ArrowDownIcon from "../../../shared/icons/arrow-down-icon";
import DuplicateIcon from "../../../shared/icons/duplicate-icon";
...
const MetadataForm = ({ form, metadata }: MetadataProps) => {
  const { control, path, register, setValue, getValues, setFocus } = form;
  ...
  return (
    <>
      <div className="rounded-rounded border-grey-20 divide-grey-20 inter-base-regular divide-y border">
        <div className={clsx("inter-small-semibold bg-grey-5 rounded-t-rounded", rowClasses)}>
          <div>
            <p>Key</p>
          </div>
          <div className="">
        ...
        <div className="divide-grey-20 divide-y">
          {!fields.length ? (
            ...
            >
              <div>
                <MetadataInput {...register(path(`entries.${0}.key`))} placeholder="Key" />
              </div>
              <div>
                <MetadataInput {...register(path(`entries.${0}.value`))} placeholder="Value" />
              </div>
            </MetadataRow>
          ) : (
            fields.map((field, index) => {
              return (
                ...
                >
                  <div>
                    <MetadataInput {...register(path(`entries.${index}.key`))} placeholder="Key" />
                  </div>
                  <div>
                    <MetadataInput {...register(path(`entries.${index}.value`))} placeholder="Value" />
                  </div>
                </MetadataRow>
              );
            })
      ...
      {ignoredLength > 0 && (
        <div className="rounded-rounded p-base gap-x-base mt-base flex items-start border border-[#FFD386] bg-[#FFECBC]">
          <div>
            <WarningCircleIcon fillType="solid" size={20} className="text-[#FFB224]" />
          </div>
          <div>
            <p className="inter-small-regular text-[#AD5700]">
              This entities metadata contains complex values that we currently don&apos;t support editing through the
              admin UI. Due to this {Object.keys(ignoredLength)} keys are currently not being displayed. You can still
...
const MetadataInput = React.forwardRef<HTMLInputElement, React.ComponentPropsWithoutRef<"input">>(
  ({ className, ...props }, ref) => {
    return (
      <input
        ref={ref}
      ...
      <DropdownMenu.Root>
        ...
        <DropdownMenu.Content asChild sideOffset={8}>
          <div className="bg-grey-0 shadow-dropdown border-grey-20 rounded-rounded z-50 overflow-hidden border">
            ...
            <DropdownMenu.Item onClick={onClearContents} className={itemClasses}>
              <XCircleIcon size={20} />
...
export const getSubmittableMetadata = (data: MetadataFormType): Record<string, unknown> => {
...
export const getMetadataFormValues = (metadata?: Record<string, any> | null): MetadataFormType => {
