...
const Settings: React.FC<SettingsProps> = ({ promotion, isEdit = false }) => {
  const { register, control, isDynamic, hasExpiryDate, handleConfigurationChanged, isAdminCoupon, setValue } =
    useDiscountForm();
  ...
  useEffect(() => {
    if (isAdminCoupon) {
      setOpenItems([...openItems, "usage_limit"]);
      ...
      >
        {render && (
          <>
            ...
            >
              <div
                className={clsx("gap-xsmall flex items-center", marginTransition, {
