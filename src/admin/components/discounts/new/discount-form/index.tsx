...
import MetadataForm from "../../../forms/general/metadata-form";
...
const DiscountForm = ({ closeForm, metadata }: DiscountFormProps) => {
  const { handleSubmit, handleReset, control, form, setValue, setIsAdminCoupon } = useDiscountForm();
  ...
  useEffect(()=>{
    setValue('rule.type', DiscountRuleType.FIXED)
    setIsAdminCoupon(true);
  },[metadata])
    ...
    <FocusModal>
      <FocusModal.Header>
        <div className="medium:w-8/12 flex w-full justify-between px-8">
          <Button size="small" variant="ghost" onClick={closeForm} className="rounded-rounded h-8 w-8 border">
            <CrossIcon size={20} />
          </Button>
          <div className="gap-x-small flex">
            <Button onClick={handleSubmit(submitGhost)} size="small" variant="ghost" className="rounded-rounded border">
      ...
      <FocusModal.Main>
        <div className="mb-[25%] flex justify-center">
          <div className="w-full max-w-[700px] pt-16">
            <h1 className="inter-xlarge-semibold">{t("discount-form-create-new-discount", "Create new discount")}</h1>
            <Accordion className="text-grey-90 pt-7" defaultValue={["promotion-type"]} type="multiple">
              <Accordion.Item
                forceMountContent
              ...
              <Accordion.Item title={t("discount-form-general", "General")} required value="general" forceMountContent>
                <General />
              </Accordion.Item>
              <Accordion.Item
                forceMountContent
              ...
              >
                <div className="mt-small">
                  <MetadataForm form={nestedForm(form, "metadata")} metadata={metadata} />
                </div>
              </Accordion.Item>
            </Accordion>
          </div>
