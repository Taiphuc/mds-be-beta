    ...
    <components.Option {...props} className="bg-grey-0 active:bg-grey-0 my-1 py-0 px-2">
      <div className={`item-renderer hover:bg-grey-10 h-full cursor-pointer rounded py-2 px-2`}>
        <div className="flex h-full items-center">
          {data?.value !== "all" && data?.label !== "Select All" ? (
            <>
              {props.isMulti ? (
              ...
              <span className="text-grey-90 inter-base-regular ml-3">{data?.label}</span>
            </>
          ) : (
            <span className="text-grey-90 inter-base-regular">{data?.label}</span>
          )}
        </div>
      </div>
    </components.Option>
