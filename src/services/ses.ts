...
import SesSettingRepository from "src/repositories/ses-setting";
...
class SESService extends NotificationService {
  ...
  protected readonly sesSettingRepository_: typeof SesSettingRepository;
  ...
  constructor({
    ...
    super();
    this.sesSettingRepository_ = manager.withRepository(sesSettingRepository);
    this.notificationDataService_ = notificationDataService;
    this.init()
  }

  async init() {
    const settings = await this.sesSettingRepository_.find();
    this.options_ = {
      access_key_id: settings[0].key_id,
      secret_access_key: settings[0].access_key,
      region: settings[0].region,
      from: settings[0].from,
      enable_endpoint: process.env.SES_ENABLE_ENDPOINT,
      enable_sim_mode: Boolean(process.env.SES_ENABLE_SIM_MODE),
      ...
      fs.readdirSync(this.partialPath_).forEach((filename) => {
        if (filename.endsWith(".hbs")) {
          ...
          Handlebars.registerPartial(
            name,
            Handlebars.compile(fs.readFileSync(path.join(this.partialPath_, filename), "utf8"))
          );
        }
      });
    }
  ...
  async sendNotification(event, eventData, attachmentGenerator) {
    ...
    const attachments = await this.notificationDataService_.fetchAttachments(event, data, attachmentGenerator);
  ...
  async sendEmailWithoutTemplate(payload: { from: string; to: string; subject: string; html: string; text: string }) {
    // This function is used by the /ses/send API endpoint included in this plugin.
    // The endpoint is disabled by default.
    try {
      return this.transporter_.sendMail({
  ...
  async compileTemplate(templateId, data) {
    const subjectTemplate = fs.existsSync(path.join(this.templatePath_, templateId, "subject.hbs"))
      ? Handlebars.compile(fs.readFileSync(path.join(this.templatePath_, templateId, "subject.hbs"), "utf8"))
      : null;

    const htmlTemplate = fs.existsSync(path.join(this.templatePath_, templateId, "html.hbs"))
      ? Handlebars.compile(fs.readFileSync(path.join(this.templatePath_, templateId, "html.hbs"), "utf8"))
      : null;

    const textTemplate = fs.existsSync(path.join(this.templatePath_, templateId, "text.hbs"))
      ? Handlebars.compile(fs.readFileSync(path.join(this.templatePath_, templateId, "text.hbs"), "utf8"))
      : null;
  ...
  async getTemplate(templateId) {
    const subjectTemplate = fs.existsSync(path.join(this.templatePath_, templateId, "subject.hbs"))
      ? fs.readFileSync(path.join(this.templatePath_, templateId, "subject.hbs"), "utf8")
      : null;

    const htmlTemplate = fs.existsSync(path.join(this.templatePath_, templateId, "html.hbs"))
      ? fs.readFileSync(path.join(this.templatePath_, templateId, "html.hbs"), "utf8")
      : null;

    const textTemplate = fs.existsSync(path.join(this.templatePath_, templateId, "text.hbs"))
      ? fs.readFileSync(path.join(this.templatePath_, templateId, "text.hbs"), "utf8")
      : null;
